...
(function () {
    ...
    function replaceDownloadLink(link, http1) {
        var http = http1 || false;
        //是否强制将https替换为http
        if (http) {
        ...
        function downloadClick(event) {
            ...
            if (id == 'download-direct') {
                ...
                if (id == 'download-api') {
                    ...
                    var result1 = getDownloadLinkWithClientAPI(selectFileList[0].path);
                    if (result1.errno == 0) {
                        downloadLink = result1.urls[0].url;
                    } else if (result1.errno == 1) {
                        alert('文件不存在！');
                        return;
                    } else if (result1.errno == 2) {
        ...
        function linkClick(event) {
            ...
            if (id.indexOf('direct') != -1) {
                ...
                if (id.indexOf('api') != -1) {
                    ...
                    result = getDownloadLinkWithClientAPI(selectFileList[0].path);
                    if (result.errno == 0) {
                        linkList = {
                            filename: selectFileList[0].filename,
                            urls: result.urls
    ...
    function getLogID() {
        ...
        function l(e) {
            ...
            var n2 = 65536 + 1024 * (e.charCodeAt(0) - 55296) + (e.charCodeAt(1) - 56320);
            return f(240 | n2 >>> 18 & 7) + f(128 | n2 >>> 12 & 63) + f(128 | n2 >>> 6 & 63) + f(128 | 63 & n2);
        }
